...
class TransitionCallback:
    def on_transition(self, from_state, to_state, inputs, outputs):
        st.write(f"Transitioning from {from_state} to {to_state}")
        print(f"LANGCHAIN: Transitioning from {from_state} to {to_state}")
        # Custom action
...
class CustomLangGraph(StateGraph):
    ...
    def transition(self, from_state, to_state, inputs):
        outputs = super().transition(from_state, to_state, inputs)
        self.callback.on_transition(from_state, to_state, inputs, outputs)
        return outputs
...
# Build graph
#
workflow = CustomLangGraph(GraphState,callback=callback)
...
def main():
    st.write("# Email Responder")
    if EMAIL := st.text_area(f"Type in customer email here",height=200):
